...
# (Complete file content with corrected provider instantiation in __init__)
...
class RAGWorkflowManager:
    ...
    def __init__(self,
        ...
        Allows overriding LLM provider and model from the main configuration,
        enabling dynamic configuration based on user selection or other runtime needs.
        """
        self.config = config # Store main config for non-LLM settings & API keys
        ...
        try:
            ...
            effective_provider_name = provider_override if provider_override else self.config.get_llm_provider_name()
            effective_model_name = model_override if model_override else self.config.get_llm_model_name()
            logger.info(f"Initializing LLM provider and interaction for: "
                        f"Provider='{effective_provider_name}', Model='{effective_model_name}'")
            # ---------------------------------------------

            # --- CORRECTED PROVIDER INSTANTIATION ---
            # Call the factory, passing the override name.
            # The factory uses the override name but gets the key from the main config.
            self._llm_provider = self.llm_provider_factory.create_provider(
                self.config,
                provider_name_override=effective_provider_name
            )
            if self._llm_provider is None:
                raise LLMProviderError(f"LLM Provider could not be created for '{effective_provider_name}'.")
            # -----------------------------------------

            # Create interaction objects using the *correctly instantiated* provider
            main_llm_kwargs = {'temperature': self.config.get_llm_temperature()}
            self._llm_interaction = self._llm_provider.create_interaction(effective_model_name, **main_llm_kwargs)
            logger.info(f"Main LLM Interaction ({effective_model_name}) ready using provider {self._llm_provider.get_provider_name()}.")

            router_model_name = effective_model_name # Use same model for router for now
            ...
            logger.info(f"Router LLM Interaction ({router_model_name}) ready using provider {self._llm_provider.get_provider_name()}.")
            ...
            logger.error(f"Fatal: Failed to initialize LLM components or Router for '{effective_provider_name}/{effective_model_name}': {e}", exc_info=True)
    ...
    # (_build_question_router, node functions, edge functions, build_graph,
    #  get_graph_visualization_png, run_workflow, _create_result_from_state)
    # ... (Paste the rest of the RAGWorkflowManager class code from the previous correct response #35 here)
    # --- Start pasting from previous correct version here ---
...
Choose only 'vectorstore' or 'web_search' as the datasource."""
    ...
    def _retrieve_vectorstore_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            state.current_retrieval_strategy = retriever.get_name() # Set strategy name
    ...
    def _grade_documents_node(self, state: WorkflowState) -> WorkflowState:
        ...
        retrieval_source = state.current_retrieval_strategy
    ...
    def build_graph(self) -> CompiledGraph:
        ...
        logger.info("Building RAG workflow graph (web search routes to grading)...")
        ...
        try:
            ...
            # Only cache if this is the *default* instance, not a temporary one
            # Logic in RAGApplication now handles temporary instances, so we can cache here
            # if we add a check or assume this init is only called once for the default manager.
            # Let's keep caching disabled for now to support the temporary manager pattern.
            # self._compiled_graph = compiled_graph
            # self._is_graph_built = True
            logger.info("Workflow graph compiled successfully (web search now routes to grading).")
            if compiled_graph is None: raise WorkflowError("Graph compilation returned None unexpectedly.")
    ...
    def get_graph_visualization_png(self, compiled_graph_instance: Optional[CompiledGraph] = None) -> Optional[bytes]:
        target_graph = compiled_graph_instance
        # If no specific instance passed, try to build/get default (might not be ideal if called on temp instance)
        if not target_graph:
            try:
                 logger.info("No compiled graph passed for visualization. Building default graph.")
                 target_graph = self.build_graph()
                 if not target_graph: logger.warning("Default graph compilation failed during visualization request."); return None
            except Exception as e: logger.error(f"Failed to build graph for visualization: {e}", exc_info=False); return None

        logger.info("Attempting to generate workflow graph PNG using draw_png / Graphviz...")
        try:
            if target_graph is None: logger.error("Cannot generate graph PNG: Compiled graph is None."); return None
            png_bytes = target_graph.get_graph(xray=True).draw_png()
            if png_bytes: logger.info(f"Successfully generated graph PNG data using draw_png ({len(png_bytes)} bytes)."); return png_bytes
            else: logger.warning("draw_png() returned None or empty bytes without raising an exception."); return None
        except ImportError as ie: logger.error(f"ImportError during Graphviz rendering: {ie}. Is 'pygraphviz' still installed?", exc_info=True); return None
        except FileNotFoundError as fnf_e: logger.error(f"Graphviz FileNotFoundError: {fnf_e}. Can't find 'dot' executable.", exc_info=True); return None
        except Exception as e: logger.error(f"Failed to draw workflow graph using draw_png: {e}", exc_info=True); return None
    ...
    def run_workflow(self, question: str, initial_source_urls: Optional[List[str]] = None) -> Result:
        ...
        try:
            ...
            if not compiled_graph: raise WorkflowError("Graph compilation failed or returned None.")
            logger.info("Attempting to generate graph visualization PNG after compilation...")
            try: graph_png_bytes = self.get_graph_visualization_png(compiled_graph_instance=compiled_graph)
            except Exception as graph_err: logger.error(f"Failed to generate graph visualization: {graph_err}", exc_info=False)
        ...
        if initial_source_urls:
             initial_url_tracking = {}; current_time_utc = datetime.now(timezone.utc).isoformat()
        ...
        try:
            ...
            if final_state_dict and isinstance(final_state_dict, dict):
                 ...
                 except TypeError as te: logger.error(f"Mismatch between final state dict and WorkflowState dataclass: {te}. Keys: {list(final_state_dict.keys())}"); raise WorkflowError(f"Could not reconstruct final WorkflowState: {te}")
            else: logger.error(f"Workflow execution returned unexpected type: {type(final_state_dict)}"); raise WorkflowError("Workflow execution did not return a valid final state dictionary.")
        except Exception as e:
            logger.error(f"Workflow execution failed: {e}", exc_info=True)
            logs_on_fail = []
            if isinstance(initial_state_input.get('logs'), list): logs_on_fail = [LogEntry(**log) for log in initial_state_input['logs'] if isinstance(log, dict)]
        ...
        if final_state is None: logger.error("Final state object is None after workflow invocation."); return Result(question=question, answer_text="Workflow failed: Final state object missing.", execution_metadata={"error": "Final state object missing"}, graph_diagram_data=graph_png_bytes)
        result = self._create_result_from_state(final_state, graph_png_bytes)
        end_run_time = time.time()
        if isinstance(result.execution_metadata, dict): result.execution_metadata["total_duration_sec"] = round(end_run_time - start_run_time, 3)
        else: logger.warning("Execution metadata is not a dict, cannot add duration.")
    ...
    def _create_result_from_state(self, final_state: WorkflowState, graph_diagram_data: Optional[bytes] = None) -> Result:
        ...
        if isinstance(final_state.url_usage_tracking, dict):
            for url, usage_info in final_state.url_usage_tracking.items():
                ...
                elif not isinstance(usage_info, UrlUsageInfo): logger.warning(f"Skipping invalid usage_info for URL '{url}': Type {type(usage_info)}"); continue
                ...
                source_summary.append(SourceInfo(url=url, source_type=getattr(usage_info, 'source_type', "unknown"), final_relevance_metric=final_relevance, usage_count=getattr(usage_info, 'count', 0)))
        else: logger.warning("Final state url_usage_tracking is not a dict.")
        log_entries = []
        ...
        else: logger.warning("Final state logs is not a list.")
        exec_meta = {
        ...
        return Result(question=final_state.original_question, answer_text=final_state.answer or "No answer was generated.", final_source_summary=source_summary, log_entries=log_entries, graph_diagram_data=graph_diagram_data, execution_metadata=exec_meta)

    # --- End pasting from previous correct version here ---